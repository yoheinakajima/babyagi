...
Hints for these prompts:
   - The wording in STOP_CRITERIA has a significant impact on when this condition will be met. For example see "When the ultimate objective has been achieved and is plausible." vs. "... has been achieved in an adequate manner and is plausible". This makes a big difference, since in the second case the LLM will determine on its own what is "adequate" in this case. The part ", or when further evaluation will most probably not improve the result" has been added as a safety condition to make sure that at some point the condition will be met.
